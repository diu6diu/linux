...
int boot_relocate_fdt(struct lmb *lmb, char **of_flat_tree, ulong *of_size)
{
    ...
    if(CPU_STM32MP157Axx == get_cpu_type()) {
        cpu_node = fdt_path_offset(fdt_blob, "/cpu0-opp-table/opp-800000000");
        fdt_del_node(fdt_blob, cpu_node);
        cpu_node = fdt_path_offset(fdt_blob, "/cpu0-opp-table/opp-400000000");
        fdt_del_node(fdt_blob, cpu_node);
	    fdt_setprop_u32(fdt_blob, nodeoffset, "opp-supported-hw", 1);
    }

    if(CPU_STM32MP157Dxx == get_cpu_type()) {
        cpu_node = fdt_path_offset(fdt_blob, "/cpu0-opp-table/opp-650000000");
        fdt_del_node(fdt_blob, cpu_node);
    }
